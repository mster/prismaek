...
function isHex(hexString) {
    if (hexString?.constructor?.name !== "String") return false;
...
function isRGB(rgbObj) {
    if (
        ...
        !isNaN(rgbObj?.r) && !isNaN(rgbObj?.g) && !isNaN(rgbObj?.b) &&
        (+rgbObj?.r) < 256 && (+rgbObj?.r) >= 0 &&
        (+rgbObj?.g) < 256 && (+rgbObj?.g) >= 0 &&
        (+rgbObj?.b) < 256 && (+rgbObj?.b) >= 0
    ) {
        return true;
    }
...
function isHSV(hsvObj) {
    if (
        ...
        !isNaN(hsvObj?.h) && !isNaN(hsvObj?.s) && !isNaN(hsvObj?.v) &&
        (+hsvObj?.h) < 360 && (+hsvObj?.h) >= 0 &&
        (+hsvObj?.s) <= 1 && (+hsvObj?.s) >= 0 &&
        (+hsvObj?.v) <= 1 && (+hsvObj?.v) >= 0
    ) {
        return true;
    }
